...
public class Sardas{
	...
	int index=0;
	int current=-1;
	//dist = sqrt( (a.x-r.x)^2 + (a.y-r.y)^2 )
	
	for(Node a : Q){
	    float temp = (float)Math.sqrt(Math.pow(a.x-r.x,2)+Math.pow(a.y-r.y,2));
	    if(temp < dist){
		current = index;
		dist = temp;
	    }
	...
	//System.err.println("Removing "+rem.x + " "+rem.y);
        return rem;
    }
	...
	for(Node a : g.nodes){
	    Q.addLast(a);
	    //System.out.println("Adding "+a.x+" "+a.y);
	}
	//System.err.println("Removing First Element "+Q.get(0).x + " "+Q.get(0).y);
	...
	while(Q.size()>0){
		...
		if(belongsTo(v) && Float.compare(weight(u,v),v.dist) < 0){
		    v.pai = u;
		    v.dist = weight(u,v);
		    //System.err.println("Sum = Sum + "+v.dist);		    
		}
	    }
	...
	for(Node v : g.nodes){
	    ...
	    try{
	    System.out.println("Pai: "+v.pai.x +" "+ v.pai.y);
	    }catch(Exception e){}
