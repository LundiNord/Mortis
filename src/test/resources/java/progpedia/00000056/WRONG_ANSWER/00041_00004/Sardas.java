//Realizado com a ajuda de um antigo aluno de LCC
		...
		while (q.size() > 0) {
			...
			for (int w : n1.adj)
				if (nodes[w].visited != true)
					...
					if (/*q.contains(nodes[w]) && */(Dist(n1, nodes[w]) < nodes[w].distance)) {
						//q.remove(nodes[w]);
...
public class Sardas {
	public static void main(String args[]) {
		...
		/*
		 * for (int i=1; i<=n; i++) { System.out.print(i+" :"); for(int j :
		 * g.nodes[i].adj) System.out.print(" " + j + " " +
		 * g.nodes[j].distance); System.out.println(); }
		 */
