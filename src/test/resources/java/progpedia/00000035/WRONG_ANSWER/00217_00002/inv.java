...
public class inv{
	...
	if (start<end) {                 // Parar quando tamanho do array < 2
	    ...
	    cont= cont+ mergesort(v, start, middle);   // Ordenar metade esquerda
	    cont= cont+ mergesort(v, middle+1, end);   // Ordenar metade direita
	    cont= cont+ merge(v, start, middle, end);  // Combinar duas metades ordenadas
	}
	return cont;
    }
