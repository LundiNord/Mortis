...
public class p05{
    ...
    static void mergesort(int v[], int start, int end) {
		...
		if (start<end) {                 // Parar quando tamanho do array < 2
	    	...
	    	mergesort(v, start, middle);  count+=1; // Ordenar metade esquerda
	    	mergesort(v, middle+1, end);  count+=1;// Ordenar metade direita
	    	merge(v, start, middle, end);  count+=1;// Combinar duas metades ordenadas
		}
    }
