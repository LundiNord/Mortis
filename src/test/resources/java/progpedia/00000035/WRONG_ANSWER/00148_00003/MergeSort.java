...
public class MergeSort {
	...
	if (start<end) {                 // Parar quando tamanho do array < 2
	    ...
	    conta += mergesort(v, start, middle);   // Ordenar metade esquerda
	    conta += mergesort(v, middle+1, end);   // Ordenar metade direita
	    conta += merge(v, start, middle, end);  // Combinar duas metades ordenadas
	}
	return conta;
    }
