...
public class p05{
    ...
    static int mergesort(int v[], int start, int end) {
		...
		if (start<end) {     // Parar quando tamanho do array < 2
	    	...
	    	count = 1 + mergesort(v, start, middle);   // Ordenar metade esquerda
	    	count = 1 + mergesort(v, middle+1, end);   // Ordenar metade direita
	    	count = 1 + merge(v, start, middle, end); // Combinar duas metades ordenadas
		}
		else 
			count = 1;
