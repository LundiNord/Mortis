...
public class ContInv {
	...
	while (p1<=middle && p2<=end) {            // Enquanto de para comparar
		...
		else                {aux[i++] = v[p2++]; total=total+middle-p1+1;	}
	}
	...
	if (start<end) {                 // Parar quando tamanho do array < 2
		...
		total=total+mergesort(v, start, middle);   // Ordenar metade esquerda
		total=total+mergesort(v, middle+1, end);   // Ordenar metade direita
		total=total+merge(v, start, middle, end);  // Combinar duas metades ordenadas
		}
	return total;	
    }
