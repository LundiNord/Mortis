...
public class NegocioElectronico {
	...
	public static int construirGrafo(){
		...
		while(inicio!=-1){
			...
			g.insert_new_arc(inicio, fim, distancia);
			g.insert_new_arc(fim, inicio, distancia);
	...
	public static void Dijkstra(int s){
		...
		while(!Q.isEmpty()){
			...
			if(v!=Integer.MAX_VALUE){
				...
				for(Arco arco: g.adjs_no(v)){
					w=arco.extremo_final();
					dvw=arco.valor_arco();
					
					if(dist[w]>(dist[v]+dvw) && g.verts[w].visitado==false){
						dist[w]=dist[v]+dvw;
