...
public class NegocioElectronico {
	...
	public static void construirGrafo(Grafo g, Scanner kb){
		...
		while((no1 = kb.nextInt()) != -1){
			no2=kb.nextInt();
			distancia=kb.nextInt();
			g.insert_new_arc(no1, no2, distancia);
			g.insert_new_arc(no2, no1, distancia);
		}		
	}
	...
	public static void main(String[] args) {
		...
		int numeroLojas = kb.nextInt();
		int destino = kb.nextInt();

		Grafo g = new Grafo (numeroLojas);
		construirGrafo(g,kb);
		Dijkstra(g,destino);
		...
		for(int i=0; i<=g.num_vertices();i++){
			...
			for(int j=0; j<g.verts[i].adjs.size();j++){
				System.out.print("no final = "+ g.verts[i].adjs.get(j).no_final +" " + "dist = "+g.verts[i].adjs.get(j).distancia);
			}
			System.out.println();
		}*/
