...
class Node {
    ...
    public int distance;            // Distancia do nó origem da pesquisa
	public int exc; 				// Guarda a excentricidade do nó
...
public class prob19 {
    public static void main(String args[]) {
		...
		//Imprime os nós centrais (nós com o mesmo raio)
		boolean first = true;
		for(int i=1; i<=N; i++)
			if(g.nodes[i].exc == raio){
		...
		//Imprime os nós periféricos (nós com o mesmo diametro)
		first = true;
		for(int i=1; i<=N; i++)
			if(g.nodes[i].exc == diametro){
				if (first) first=false;
