...
public class nineteen {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++)
			if(g[i].ex==raio)
				No_central.add(i+1);
			for (int i = 0; i < No_central.size(); i++){
				if(i==No_central.size()-1)
					System.out.println(No_central.get(i));
				else System.out.print(No_central.get(i)+" ");
			}

		for (int i = 0; i < n; i++)
			if(g[i].ex==diametro)
				No_periferico.add(i+1);
		for (int i = 0; i < No_periferico.size(); i++){
			if(i==No_periferico.size()-1)
				System.out.println(No_periferico.get(i));
			else System.out.print(No_periferico.get(i)+" ");
		}
	}
}
