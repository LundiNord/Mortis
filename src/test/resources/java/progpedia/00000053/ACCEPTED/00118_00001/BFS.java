...
public class BFS {
	...
	Graph g = new Graph(numNos);
	int   numArestas = in.nextInt();
	for (int i=0; i<numArestas; i++) 
	    g.addLink(in.nextInt(), in.nextInt());

	int matrizDist[][]= new int[numNos+1][numNos+1];
	// Pesquisa em largura a partir do no 1
	
	Node nos[]= new Node[numNos+1]; 
	for (int i=1; i<=numNos; i++) 
		nos = g.bfs(i, matrizDist);
	
	int max;
	for (int i=1; i<=numNos; i++){
		max = 0;
		for (int j=1; j<=numNos; j++)
			if (max < matrizDist[i][j]){
				max = matrizDist[i][j];
			}
		nos[i].exc = max;
	}
