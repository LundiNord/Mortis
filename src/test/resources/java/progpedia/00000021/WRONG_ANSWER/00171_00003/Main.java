	...
	public static void main(String[] args) {
		...
		while(local1 != -1){
			...
			if(lMaxArco >= lMin && cMaxArco >= cMin && alturaArco >= aMin){
				...
				if((origem == local1 || origem == local2) && (destino == local1 || destino == local2))contagem++;
			}

			local1 = scan.nextInt();
		}
		if(contagem == 0) System.out.println("0");
		else{
			System.out.println(locaisGrafo.Dijkstra(origem, destino));
		}
	}
