...
class Distancia
{
	private int largura;
		...
		public void definirLargura(int largura){ this.largura = largura;}
	
	private int comprimento;
		public int obterComprimento(){return comprimento;}
		public void definirComprimento(int comprimento){ this.comprimento = comprimento;}
		
	private int altura;
		public int obterAltura(){return altura;}
		public void definirAltura(int altura){this.altura =altura;}
	...
	{
		return new Distancia(largura,comprimento,altura);
	}
...
class Grafo
{
	...
	public void encontrarRecursivo(HashMap<Integer,Void> visitadas, int actual, int ultimo,DistanciaAnimal disAni,Distancia dist )
	{	
		
		if(actual == ultimo)
		{			
			
			if (this.melhorTempoFinal < dist.obterComprimento())
			{
				this.melhorTempoFinal = dist.obterComprimento();
			}		
		...
		else
		{
			...
			for(Integer ligacao : ligacoes)
			{
				if(!visitadas.containsKey(ligacao))
				{
					...
					Distancia distCarrinha = dist.clone();
					
					if(dis.obterComprimento() < distCarrinha.obterComprimento())
					{
						distCarrinha.definirComprimento(dis.obterComprimento() );
						distCarrinha.definirAltura(dis.obterAltura());
						distCarrinha.definirLargura(dis.obterLargura());
					}
					
					if(
						(disAni.obterLargMin() <= distCarrinha.obterLargura()) && (distCarrinha.obterLargura() <= disAni.obterLargMax())
						&& (disAni.obterCompMin() <= distCarrinha.obterComprimento()) && (distCarrinha.obterComprimento() <= disAni.obterCompMax())
						&& (disAni.obterAltMin() <= distCarrinha.obterAltura())
					  )
					{
						HashMap<Integer,Void> visitadasClone = (HashMap<Integer,Void>) visitadas.clone();
						visitadasClone.put(ligacao, null);
						
						encontrarRecursivo(visitadasClone,ligacao,ultimo,disAni,distCarrinha);
...
public class Main 
{
	...
	public static void main(String[] args) 
	{
		...
		Distancia dist = new Distancia(0,new Integer(Integer.MAX_VALUE),0);
		
		grafo.encontrarRecursivo(visited,comeco,fim,disAni, dist );
		if(grafo.melhorTempoFinal == Integer.MAX_VALUE)
			grafo.melhorTempoFinal = 0;
