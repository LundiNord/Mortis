...
public class main {
	...
	public static void prim(Grafo g, int s) {
		...
		if (flag == 0)
			System.out.println("rendimento optimo: " + custo);
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= nLigacoes; i++) {
			int ext1 = stdin.nextInt();
			if (index == 0)
				first = ext1;
			index = 1;
			int ext2 = stdin.nextInt();
			int rend = stdin.nextInt();
			g.insert_new_arc(ext1, ext2, rend - custoManutencao);
			g.insert_new_arc(ext2, ext1, rend - custoManutencao);
		}
		prim(g, first);
