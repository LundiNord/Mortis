...
class m{ 
	...
	*/
	...
	for(int i=0; i<p; i++){
	    ...
	    if((pi-resul[lowmaior-1])<(resul[lowmaior]-pi)&& lowmaior-1>1){
		lowmaior=lowmaior-1;
	    }
	    else if((pi-resul[lowmaior-1])==(resul[lowmaior]-pi) && resul[lowmaior]!=resul[lowmaior-1] && lowmaior-1>1){
		    lowmenor=lowmaior-1;
		}
	    if(lowmenor!=0){
		if(i!=p-1){

System.out.println(resul[lowmenor] + " " +resul[lowmaior]);
}
	 
		else{System.out.print(
resul[lowmenor] + " " +resul[lowmaior]);
		    System.out.println();
}
	    }
		else{  
		    if(i!=p-1){   
System.out.println(resul[lowmaior] );
		    }
