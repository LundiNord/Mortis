			...
			for(int x = 0; x<n ; x++){
				...
				if(Math.abs(soma - list[so] - list[x]) < high && so != x){
					...
					for(int y = 0; y<index; y++){
						if(results[y] == list[x] + list[so]){
							notIn = false;
						}
					}
					
					if(notIn){
						results[index] = list[x] + list[so];
						index++;
					}
				...
				if(Math.abs(soma - list[so-1] - list[x]) < high && so-1 != x){
					...
					boolean notIn = true;
					for(int y = 0; y<index; y++){
						if(results[y] == list[x] + list[so-1]){
							notIn = false;
						}
					}
					
					if(notIn){
						results[index] = list[x] + list[so-1];
						index++;
					}
				}
