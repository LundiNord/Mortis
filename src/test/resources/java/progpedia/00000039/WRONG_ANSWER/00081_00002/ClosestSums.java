...
class ClosestSums{
	...
	public static void main(String[] args){
		...
		for(int i=0;i<nQ;i++){
			...
			if(dif!=0){
				bsearch(sums,0,sums.length-1,q[i],dif,false);
			}
			else
	...
	static void bsearch(int[] sums, int first, int last, int key, int dif,boolean flag){
		int middle;
		...
		if(first<=last){
			if(sums[middle]<key){
				...
				bsearch(sums,middle+1,last,key,dif,flag);
			}
				...
				bsearch(sums,first,middle-1,key,dif,flag);
			}
			else
