...
public class prob06 {
	...
	private static int[] somador(int[] v, int ncombos) {

				int[] sumPares = new int[ncombos]; 
				int temp = 0;
				if(temp> ncombos ) return sumPares;
				
				for (int i = 0; i < v.length; i++) { 
					for (int j = i + 1; j < v.length; j++) { 
						sumPares[temp] = v[i] + v[j];
						temp++;
					}					
				}			
		return sumPares;
	 ...
	 static int BinarySearch(int[] somas ,int key)
	{
	    ...
	    while (low < high) {
	        int mid = (low + high) / 2;
	        //assert(mid < high); //obrigado google
