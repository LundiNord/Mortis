...
public class ENC {
	...
	public ENC(HashMap<Integer, ArrayList<Integer>> HM, int [][] l , int [][] c, int [][] a,
			int lmin, int lmax,int cmin, int cmax, int alt, int p, int f, ArrayList<Integer> ll){
		hm = HM;
		largura = l;
		comprimento = c;
		altura = a;
	...
	public void  caminhos(int vertice, int fim){		
		vis.add(vertice);
		for (int i : hm.get(vertice)){
			if (vis.size()==lll.size()){
	...
	public static void main(String[] args) {
		...
		ENC enc = new ENC(hm, largura,comprimento, altura, larg_min,larg_max,comp_min,comp_max,alt_min,partida,fim, vertices); 
		enc.caminhos(partida, fim);
		enc.res();
